...
snap_to_grid <- function(xy, gr) {
  ...
  xy_new <- cbind(
    deltax[apply(xdiff, 1, which.min)],
    deltay[apply(ydiff, 1, which.min)]
  )
  dups <- duplicated(xy_new)
  while (any(dups)) {
    xy_new[which(dups), ] <- xy_new[which(dups), ] +
      c(sample(c(1, -1), 1) * gr, sample(c(1, -1), 1) * gr)
    dups <- duplicated(xy_new)
  }
  xy_new
}
