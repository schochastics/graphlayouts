...
layout_as_backbone <- function(g, keep = 0.2, backbone = TRUE) {
  ...
  if (!requireNamespace("oaqc", quietly = TRUE)) {
    stop(
      "oaqc is needed for this function to work. Please install it.",
      call. = FALSE
    )
  }
  ...
  # weighting ----
  orbs <- oaqc::oaqc(
    igraph::as_edgelist(g, names = FALSE) - 1,
    non_ind_freq = TRUE
  )
  ...
  # filtering ----
  igraph::E(g)$bone <- w >=
    sort(w, decreasing = TRUE)[ceiling(igraph::ecount(g) * keep)]
  g_bone <- igraph::graph_from_edgelist(
    el[igraph::E(g)$bone, 1:2],
    directed = F
  )
...
backbone_edges <- function(g, g_lay) {
  tmp <- rbind(
    igraph::as_edgelist(g_lay),
    igraph::as_edgelist(g, names = FALSE)
  )
